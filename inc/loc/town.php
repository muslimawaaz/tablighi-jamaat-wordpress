...
if($_POST["action"]){
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <?php
        global $wpdb;
        $district_opts = $wpdb->get_results("SELECT id,district FROM district",ARRAY_A);
        ?>
        <td><select name="district" >
        <?php
        foreach ($district_opts as $key) { 
            echo '<option value="'.$key["id"].'">'.$key["district"].'</option>';
        }
        ?>
            </select>
        </td>
    ...
    <script type="text/javascript">
        ...
        $('select[name=district]').val('<?php echo $data["district"]; ?>');
    </script>
        <?php
    }
