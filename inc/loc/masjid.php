...
if($_POST["action"]){
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <tr>
            <td>Masjid</td>
            <td><input type="text" name="masjid">
            </td>
        </tr>
        <tr><?php
                ...
                <td><select name="town">
                    ...
                    <script type="text/javascript">
                        $('select[name=town]').on('change',function(){
                            var x = $('select[name=town]').val();
                            $('select[name=halqa] option').hide();
                            $('select[name=halqa] option.prnt_'+x).show();
                        });
                    </script>
                </td>
            </tr>
            <tr><?php
                    $halqa_opts = $wpdb->get_results("SELECT id,halqa,town FROM halqa");
                    ?>
                <td>Halqa</td>
                <td><select name="halqa">
                                <option value="">Select</option>
                        <?php
                        foreach ($halqa_opts as $key) { 
                            echo '<option value="'.$key->id.'" class="prnt_'.$key->town.'">'.$key->halqa.'</option>';
                        }
                        ?>
                    </select>
                </td>
    ...
    <script type="text/javascript">
        $('input[name=masjid]').val('<?php echo $data["masjid"]; ?>');
        <?php $halqa = $data["halqa"]; ?>
        $('select[name=halqa]').val('<?php echo $halqa; ?>');
        <?php
        $town = $wpdb->get_var("SELECT town FROM halqa WHERE id = $halqa");
...
if(($_POST["action"]!='Edit') && $_POST["action"]!='Add New') {
    ...
    <table id="myTable" class="ui unstackable celled table dataTable">
        ...
        <tbody>
            ...
            $halqa_opts = $wpdb->get_results("SELECT id,halqa FROM halqa",OBJECT_K);
            foreach($rows as $row){
                echo '<tr>';
                echo '<td>'.$row->masjid.'</td>';
                echo '<td>'.$halqa_opts[$row->halqa]->halqa.'</td>';
                echo '</tr>';
            }
            ?>
        </tbody>
